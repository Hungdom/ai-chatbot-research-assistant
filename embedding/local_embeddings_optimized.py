...
OPTIMIZED_BATCH_SIZES = {
    'db_fetch': 1000,        # Increased from 100
    'embedding': 128,        # Increased from 32 (adjust based on GPU memory)
    'db_update': 500,        # Batch size for database updates
    'commit_every': 5        # Commit every N batches
}
...
class OptimizedLocalEmbedder:
    def __init__(self, model_name=MODEL_NAME, target_dim=TARGET_DIMENSIONS):
        ...
        logger.info(f"Optimized batch sizes: {OPTIMIZED_BATCH_SIZES}")
    ...
    def fit_reducer(self, sample_texts, sample_size=10000):
        ...
        # Get embeddings for sample with larger batch size
        embeddings = self.model.encode(
            sample_texts, 
            show_progress_bar=True, 
            batch_size=OPTIMIZED_BATCH_SIZES['embedding'],
    ...
    def get_embeddings(self, texts, batch_size=None):
        """Get embeddings with target dimensions - optimized version"""
        ...
        # Get original embeddings with optimizations
...
def get_text_for_embedding(row):
    """Combine relevant fields to create text for embedding - optimized version"""
    text_parts = []
...
def get_next_batch_cursor_based(cur, last_id=0, batch_size=1000):
...
def process_batch_parallel(rows, embedder):
    ...
    # Get embeddings using optimized batch size
...
def update_embeddings_optimized():
    """Optimized version of update_embeddings with major performance improvements"""
    ...
    try:
        with psycopg2.connect(**DB_CONFIG) as conn:
            with conn.cursor() as cur:
                ...
                pbar = tqdm(total=total_rows, desc="Processing embeddings", unit="records")
                ...
                while processed_records < total_rows:
                    ...
                    rows = get_next_batch_cursor_based(
                        cur, 
                        last_id=last_id, 
                        batch_size=OPTIMIZED_BATCH_SIZES['db_fetch']
                    )
                    ...
                    updates = process_batch_parallel(rows, embedder)
                    ...
                    if updates:
                        ...
                        execute_batch(
                            ...
                            updates,
                            page_size=OPTIMIZED_BATCH_SIZES['db_update']
                        )
                        
                        # Commit every N batches for better performance
                        if batch_count % OPTIMIZED_BATCH_SIZES['commit_every'] == 0:
                            conn.commit()
                    ...
                    if batch_count % 10 == 0:  # Log every 10 batches
                        logger.info(f"📊 BATCH {batch_count}: {processed_records}/{total_rows} records "
                ...
                logger.info(f"🎉 COMPLETED: Processed {processed_records} records in {total_time/60:.1f} minutes")
...
def test_embeddings_optimized():
    """Test the optimized local embedding setup"""
    logger.info("Testing optimized local embeddings...")
...
def main():
    logger.info("🚀 Starting OPTIMIZED local embedding process...")
    
    # Test setup first
    if test_embeddings_optimized():
        logger.info("✅ Optimized local embedding test passed!")
        logger.info("🏃‍♂️ Starting optimized batch processing...")
        update_embeddings_optimized()
        logger.info("🎉 Optimized local embedding process completed!")
    else:
        logger.error("❌ Optimized local embedding test failed!")
